...
- `DATABASE_URL` is the application database used by Drizzle, Better Auth and
  the app-owned domain tables.
- `TEST_DATABASE_URL` is used by DB tests only.
- `BETTER_AUTH_SECRET` signs and protects Better Auth state. Generate a local
  value with:
...
- `BETTER_AUTH_URL` is the base URL for the running app. Use
...
`npm run db:push` reads `DATABASE_URL`, so confirm `.env` points at the local
...
To test registration links locally:
