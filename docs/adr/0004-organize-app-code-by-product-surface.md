...
Event-scoped rules and master data are described in the domain as Bases del evento. Concrete modules should use direct resource names such as `event-categories`, `event-prices`, `event-modalities` and `event-schedules`; broader `event-bases` names are reserved for aggregators that genuinely coordinate several of those resources.
