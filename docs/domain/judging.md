...
- The automatic ordering sorts in blocks: schedule (date, time, then name), then category age order (minimum age, maximum age, then name), then group type in the order solo, duo, trio, grupal. Modality and submodality play no part; a schedule already restricts the modalities it accepts.
