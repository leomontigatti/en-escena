...
- A score value is between 0 and 100 and moves in steps of 0.5, which is what the panel works in. The same applies to a criterion value, between 0 and its own maximum. Both are enforced in validation and as check constraints, and a judge sees the value written with a decimal point, the deliberate exception to es-AR formatting on this surface.
