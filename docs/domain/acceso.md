...
- The registration form has fixed email from token, academy data, password and confirmation.
- If email already belongs to an existing user, public response must not reveal it.
- Completing public registration confirms email, creates academy plus academy user, authenticates the academy, and does not require admin approval.
