...
- When the destination modalidad resolves exactly one compatible cupo it is preselected and read-only, like registration. With several, choosing one is required and an empty selection is a client-side required-field error on the cupo, not a server rejection.
