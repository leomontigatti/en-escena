...
- Payment instructions (the event's bank identifiers and how-to-pay text, read by the portal payments page): `app/lib/finances/bank-identifiers.ts`, the six `paymentInstructions*` fields of `app/lib/admin/events/form-values.ts`, and the admin panel behind the event detail tabs: `app/features/admin/events/detail/payment-instructions-fields.tsx`
