...
1. **Nothing in the system merges to `master`.** A human approves and merges. The agents
...
- **Pre-fetched context.** For PR workflows the orchestrator's _runner script_ fetches the
  ...
  `` !`gh issue view …` `` / `` !`git diff master..HEAD` `` command-substitutions and a
...
- After parsing, the orchestrator's script **drops** any inline comment whose `path`+`line`
  ...
  `commentId` wasn't in the fetched threads. (Diff hunks are parsed from `git diff master...HEAD`.)
...
2. Checkout `master` (`fetch-depth: 1`); Node + deps + agent runner.
...
2. Checkout `master` (`fetch-depth: 0`) with `AGENT_PAT || GITHUB_TOKEN` (PAT lets the push
...
8. Open a **draft** PR: `gh pr create --draft --base master --head "$BRANCH" --title <title,
...
- **implement agent.** _Inputs:_ `ISSUE_NUMBER`, `ISSUE_TITLE`, `BRANCH`. Does TDD work,
  ...
  exists (`git rev-list --count master..HEAD > 0`); zero commits → write `failure_reason.txt`,
  exit 1.
...
3. Checkout `master` (`fetch-depth: 0`, `AGENT_PAT || GITHUB_TOKEN`). **Resume or create** the
...
1. Refusal preflights (above). If proceeding: remove `agent:implement` + `agent:blocked`, add
   `agent:in-progress`.
2. Checkout head SHA (`fetch-depth: 0`, `AGENT_PAT || GITHUB_TOKEN`); ensure `master` available;
...
1. Checkout `master` (`fetch-depth: 1`); Node + deps + agent runner.
...
7. **Conflict.** `master` has since moved; PR #914 now conflicts. The maintainer labels it
   `agent:update-branch`. [Update Branch](#46-update-branch) merges `master` in; conflicts exist,
...
- **Diff-anchor validation** = `parse-diff-lines.ts` builds a `Map<path, Set<line>>` from
  `git diff master...HEAD` so hallucinated inline anchors can be dropped.
...
- [`issue-tracker.md`](./issue-tracker.md) — `gh` CLI conventions for issues/PRDs (our
  equivalent of the reference's `backlog.md`).
- [`domain.md`](./domain.md) + `CONTEXT.md` + `docs/adr/` — the repo's domain vocabulary and
  recorded decisions the agents must not contradict.
- Promote Queued (§4.7) has no separate repo doc; its behaviour is fully specified in this
  document.
