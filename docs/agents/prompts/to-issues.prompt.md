...
1. Fetch the PRD and read it carefully with `gh issue view {{PRD_NUMBER}} --comments`. It is
   the spec — do not add scope or redesign. If it is ambiguous, make the most reasonable
   interpretation and proceed; do not stop to ask.
2. Read the repo's domain/architecture docs so titles and bodies use the project's
   vocabulary: `CONTEXT.md`, `docs/adr/`, and [`docs/agents/domain.md`](../domain.md).
