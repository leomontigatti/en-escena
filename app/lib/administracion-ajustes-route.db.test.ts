...
describe("administracion/ajustes route", () => {
  ...
  test("requires confirmation to delete a Bloque horario from detail", async () => {
    const event = await createSavedEvent("Regional 2026");
    const modality = await createModality(event.id, { name: "Jazz" });
    ...
    const deleteRequest = await createSignedInRequest({
      email: "admin.confirma.bloque@example.com",
      role: "admin",
      requestUrl: `http://localhost/administracion/ajustes/bloques-horarios/${scheduleBlock.id}?evento=${event.id}`,
      body: formData({
        intent: "delete-schedule-block",
    ...
    await expect(action(routeArgs(deleteRequest.request))).resolves.toEqual({
      status: "error",
      message: "Confirmá el borrado del Bloque horario antes de continuar.",
      fieldErrors: {
        confirmDelete: "Marcá la confirmación para borrar el Bloque horario.",
      },
    });
  });
