...
describe("finance consistency across admin and portal surfaces", () => {
  test("keeps summaries, row amounts, and choreography details consistent across mixed choreography states", async () => {
    ...
    await insertImpagaInscription(currentPriceChoreography.id, owner.academyId);
    // missingPrice: unpaid with no applicable duo price.
    await insertImpagaInscription(missingPriceChoreography.id, owner.academyId);
